...
exports.pixiv = class pixiv {
    constructor() {
        this.illust = async function illust(nuke) {
            let hasil = fetcher.getJSON(`https://www.pixiv.net/ajax/illust/${nuke}`)
            return hasil
        }
        
        this.search = async function search(keyword) {
            let hasil = fetcher.getJSON(`https://www.pixiv.net/ajax/search/top/${encodeURIComponent(keyword)}`)
            
            if (hasil.novel.total = 0) hasil.error = true;
