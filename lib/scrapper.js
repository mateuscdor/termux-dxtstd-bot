...
exports.pixiv = class pixiv {
    constructor() {
        this.illust = async function illust(nuke) {
            let hasil = await fetcher.getJSON(`https://www.pixiv.net/ajax/illust/${nuke}`)
            return hasil
        }
        
        this.search = async function search(keyword) {
            let hasil = await fetcher.getJSON(`https://www.pixiv.net/ajax/search/top/${encodeURIComponent(keyword)}`)
            
            if (hasil.body.novel.total = 0) hasil.error = true;
...
exports.NHentai = class Nhentai {
    constructor() {
        ...
        const checkValid = async function checkValidNhentai(URL) {
            let error = false;
            try {
                let res = await fetcher.getWeb(URL);
        ...
        const getInfo = this.getInfo = async function getInfo(nuke = "") {
            ...
            const resultInfoNH = await fetcher.getJSON('https://nhentai.net/api/gallery/' + codeNuke)
        ...
        search.keyword = async function (keyword, page = 1) {
            ...
            if (fetchNF) throw new Error(fetchNF);
            let website = await fetcher.getWeb('https://nhentai.net/search?q=' + keyword + "&page=" + page);
        ...
        search.tag = async function (tag, page = 1) {
            ...
            if (fetchNF) throw new Error(fetchNF);
            let website = await fetcher.getWeb('https://nhentai.net/tag/' + tag + "/?page=" + page);
        ...
        search.char = async function (character, page = 1) {
            ...
            if (fetchNF) throw new Error(fetchNF);
            let website = await fetcher.getWeb('https://nhentai.net/character/' + character + "/?page=" + page);
        ...
        this.download = async function (nuke) {
            ...
            let pdf = new jsPDF();
            let coverNH = await fetcher.getBuffer(resultNH.link.cover);
            pdf.addImage(coverNH, 0, 0, getSize(coverNH, pdf).width, getSize(coverNH, pdf).height);
            
            for (var i in resultNH.link.img) {
                const IMGNH = await fetcher.getBuffer(resultNH.link.img[i]);
